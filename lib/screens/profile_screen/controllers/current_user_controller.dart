import 'package:get/get.dart';
import 'package:islamic_marriage_usa_app/core/utils/app_const_functions.dart';
import 'package:islamic_marriage_usa_app/core/utils/app_urls.dart';
import 'package:islamic_marriage_usa_app/data/services/api_service.dart';
import 'package:islamic_marriage_usa_app/screens/profile_screen/models/current_user_model.dart';

class CurrentUserController extends GetxController {
  bool isLoading = false;
  CurrentUserModel? userData;

  @override
  void onInit() {
    super.onInit();
    _fetchCurrentUserData();
  }

  Future<void> _fetchCurrentUserData() async {
    _setLoading(true);

    try {
      final response = await ApiService().get(
        url: AppUrls.getCurrentUserUrl,
        headers: AppUrls.getHeaderWithToken,
      );

      if (response.success && response.data != null) {
        userData = CurrentUserModel.fromJson(response.data);
      } else {
        final errorMessage =
            response.message['message'] ?? 'Current user data read failed';
        AppConstFunctions.customErrorMessage(message: errorMessage);
      }
    } catch (error) {
      AppConstFunctions.customErrorMessage(
          message: 'Error: ${error.toString()}');
    } finally {
      _setLoading(false);
    }
  }

  void _setLoading(bool value) {
    isLoading = value;
    update();
  }
}
